...
void sendData(udpsender::vision_message *vision_writer, struct& message);
...
void sendData(udpsender::vision_message *vision_writer, struct& message) {
    ...
    udp_client_server::udp_client server(buff, IP_ADDRESS, PORT);
    ...
    if (!vision_writer->SerializeToOstream(&output)) {
        std::cerr << "Unable to send data through udp";
    } else {
